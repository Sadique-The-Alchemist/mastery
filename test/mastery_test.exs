defmodule MasteryTest do
  ...
  test "Take a quiz, manage lifecycle and persist responses" do
    ...
    assert QuizSession.active_session_for(Math.quiz_fields().title)==[]
  ...
  defp give_wrong_answer(session) do
    Mastery.answer_question(session, "wrong", &MasteryPersistence.record_persistence/2)
  end
  defp give_right_answer(session) do
    Mastery.answer_question(session, "3", &MasteryPersistence.record_persistence/2)
  end
